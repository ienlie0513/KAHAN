...
if __name__ == '__main__':
    ...
    for seed in SEEDS:
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
            ...
            # training
            model = IKAHAN(config['num_class'], word2vec_cnt, word2vec_cmt, dimred_params, args.kahan, args.deep_classifier, args.fusion, args.ihan, args.clip, args.ent_att, device, config['image_preprocessing']['clip_embed_size'], config['word2vec_dim'], config['hid_size'], max_sent=config['max_sent'], max_len=config['max_len'], max_cmt=['max_cmt'], dropout=config['dropout'])
