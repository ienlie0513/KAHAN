...
def compute_stats(df, img_dir, end):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ending', type=str, default='en', help='Ending of filename')
    ...
    # Load the data
    if args.reduced:
        gossipcop_df = pd.read_csv('./data/gossipcop' + args.version + '_no_ignore_{}_reduced.tsv'.format(args.ending), sep='\t')
        politifact_df = pd.read_csv('./data/politifact' + args.version + '_no_ignore_{}_reduced.tsv'.format(args.ending), sep='\t')
    elif args.balanced:
        gossipcop_df = pd.read_csv('./data/gossipcop' + args.version + '_no_ignore_{}_balanced.tsv'.format(args.ending), sep='\t')
        politifact_df = pd.read_csv('./data/politifact' + args.version + '_no_ignore_{}.tsv'.format(args.ending), sep='\t')
    else:
        gossipcop_df = pd.read_csv('./data/gossipcop' + args.version + '_no_ignore_{}.tsv'.format(args.ending), sep='\t')
        politifact_df = pd.read_csv('./data/politifact' + args.version + '_no_ignore_{}.tsv'.format(args.ending), sep='\t')
    ...
    gossipcop_stats = compute_stats(gossipcop_df, './data/gossipcop' + args.version + '/news_images', args.ending)
    politifact_stats = compute_stats(politifact_df, './data/politifact' + args.version + '/news_images', args.ending)
